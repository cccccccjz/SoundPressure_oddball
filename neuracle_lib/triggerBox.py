...
#   v1.0：2022-12-02， add TriggerIn
#   v2.0: 2024-04-25,  support Mac
# Copyright (c) 2020 Neuracle, Inc. All Rights Reserved. http://neuracle.cn/

import serial
import serial.tools.list_ports  # need to pip packet 'pyserial'
import time
from ctypes import *
...
class TriggerBox(object):
    """
    触发盒控制类，用于管理设备功能ID、传感器类型及设备信息
    """

    # 功能标识符常量定义（用于指令控制）
    functionIDSensorParaGet = 1      # 获取传感器参数的功能ID
    functionIDSensorParaSet = 2      # 设置传感器参数的功能ID
    functionIDDeviceInfoGet = 3      # 获取设备信息的功能ID
    functionIDDeviceNameGet = 4      # 获取设备名称的功能ID
    functionIDSensorSampleGet = 5    # 获取传感器采样数据的功能ID
    functionIDSensorInfoGet = 6      # 获取传感器信息的功能ID
    functionIDOutputEventData = 225  # 输出事件数据的功能ID（高数值可能对应特殊事件）
    functionIDError = 131           # 错误响应的功能ID

    # 传感器类型枚举常量
    sensorTypeDigitalIN = 1         # 数字输入传感器
    sensorTypeLight = 2              # 光线传感器
    sensorTypeLineIN = 3             # 线路输入传感器
    sensorTypeMic = 4                # 麦克风传感器
    sensorTypeKey = 5                # 按键传感器
    sensorTypeTemperature = 6        # 温度传感器
    sensorTypeHumidity = 7           # 湿度传感器
    sensorTypeAmbientlight = 8       # 环境光传感器
    sensorTypeDebug = 9              # 调试用传感器
    sensorTypeAll = 255              # 通配所有传感器类型（特殊标识）

    # 传感器类型到名称的映射字典（用于可读性转换）
    sensorTypeMap = {
        sensorTypeDigitalIN: 'DigitalIN',
    ...
    # 设备属性
    _deviceID = 1                   # 设备唯一标识符（默认值，可能根据实际设备修改）
    _sensor_info = []                # 传感器信息存储列表（元素结构需根据具体协议定义）
    ...
    def get_device_info(self):
        ...
        '''
        The orders of this part and others (contain | and <<) are reversed. 
        Is it wrong in this part?
        '''

        # print("HardwareVersion: %d, firmwareVersion: %d, sensorNum: %d,  ID: %d"
        #       % (HardwareVersion, FirmwareVersion, sensorSum, int(ID)))
        self._device_info = dict(HardwareVersion=HardwareVersion, FirmwareVersion=FirmwareVersion, sensorSum=sensorSum,
                                 ID=ID)
    ...
    def read(self, functionID):
        # self._device_comport_handle.flushOutput() # should delete (for read correct response)
        ...
        if message[1] != functionID:
            if message[1] == self.functionIDError:
                ...
                raise Exception(
                    "Response error: request functionID %d, return functionID  %d" % (functionID, message[1]))
    ...
    def init_microSensor_threshold(self, sensorID):
        # Not used now
        pass

    def init_microSensor(self, sensorID):
        # Not used now
        pass

    def set_lightSensor_threshold(self, threshold, sensorID=1):
        # Not used now
        pass

    def init_lightSensor(self, EventData=11, OutputChannel=69, TriggerToBeOut=1, sensorID=1):
        # Not used now
        pass
